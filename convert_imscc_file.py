...
def process_node(xml_element, resources_data, src_dir_path, dst_dir_path):
    ...
    # Check for empty folder before processing child elements
    if not xml_element.hasChildNodes():
        new_dst_dir_path = os.path.join(dst_dir_path, title)
        logger.info("Processing empty folder: {0}".format(title))
        logger.debug("Creating directory: {0}".format(new_dst_dir_path))
